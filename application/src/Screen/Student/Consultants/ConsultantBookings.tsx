...
const ConsultantBookings = ({ navigation, route }: any) => {
      ...
      const servicePromises = serviceIds.map(async (serviceId) => {
        try {
          if (serviceId && consultantId) {
            ...
            const service = serviceData.services?.find((s: any) => s.id === serviceId);
            if (service) {
  ...
  const handleOpenChat = async (booking: Booking) => {
    try {
      ...
      // Navigate to Messages screen where the chat will be available
