...
const Home = ({ navigation }: any) => {
  ...
  const handleIconPress = async (
    ...
    try {
      ...
      // Find if student has an active booking with this consultant
      ...
      if (!activeBooking) {
        // No active booking - show alert
        Alert.alert(
          'Booking Required',
          `You need to book with ${consultant.name} before you can ${iconType === 'message' ? 'send messages' : iconType === 'phone' ? 'make audio calls' : 'make video calls'}.`,
          [
            {
              text: 'Book Now',
              onPress: () => {
  ...
  useEffect(() => {
    const fetchConsultants = async () => {
      try {
        ...
        // Filter out the top consultant from the general consultants list
