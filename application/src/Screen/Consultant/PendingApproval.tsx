    ...
    useCallback(() => {
      ...
      const loadDataInline = async () => {
        ...
        try {
          ...
          // First, cleanup any invalid applications
          try {
            const cleanupResult = await cleanupInvalidApplications();
            if (cleanupResult.cleanedCount > 0) {
              console.log('PendingApproval - Cleaned up invalid applications:', cleanupResult.cleanedCount);
              showInfo(`Cleaned up ${cleanupResult.cleanedCount} invalid applications`);
            }
          } catch (cleanupError) {
            console.warn('PendingApproval - Cleanup failed (non-critical):', cleanupError);
          }
