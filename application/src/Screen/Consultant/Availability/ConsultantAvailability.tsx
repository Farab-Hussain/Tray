...
const ConsultantAvailability = ({ navigation }: any) => {
  ...
  const checkTimeOverlap = (
    ...
    const existingMinutes = existingSlots.map(slot => parseTime(slot));
    const newMinutes = newSlots.map(slot => parseTime(slot));

    // Check if any new slot overlaps with existing slots
