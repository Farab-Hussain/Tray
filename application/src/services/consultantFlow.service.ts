...
// Demo token setter (remove in production)
export const setDemoToken = async () => {
  // This is for demo purposes - in production, Firebase auth will handle tokens
...
export const getConsultantProfile = async (
  ...
  return response.data.profile;  // Backend returns { profile: {...} }
};
...
export const cleanupInvalidApplications = async (): Promise<{ message: string; cleanedCount: number }> => {
  const response = await api.post('/consultant-flow/applications/cleanup');
  return response.data;
};
