...
// Validate and get API URL from environment
const validateApiUrl = (): string => {
  // In production, API_URL must be set and cannot be localhost
  if (!__DEV__) {
    if (!API_URL) {
      throw new Error(
        'API_URL is required in production. Please set it in your .env file.',
      );
    }
    if (API_URL.includes('localhost') || API_URL.includes('127.0.0.1')) {
      throw new Error(
        'API_URL cannot be localhost in production. Please set a production API URL in your .env file.',
      );
    }
    ...
    if (!API_URL.startsWith('https://')) {
      if (__DEV__) {
        console.warn(
          '⚠️ Production API_URL should use HTTPS for security. Current URL:',
          API_URL,
        );
      }
    }
  ...
  // Return API_URL or localhost fallback (only in development)
  return API_URL || (__DEV__ ? 'http://localhost:4000' : '');
};
...
api.interceptors.request.use(async config => {
  try {
    ...
    if (user) {
      try {
        ...
        if (!token) {
          if (__DEV__) {
            console.log(
              '🔄 [Request Interceptor] Token is null, refreshing...',
            );
          }
        ...
        if (token && token.trim() !== '') {
          ...
          if (__DEV__) {
            if (config.url?.includes('/upload/')) {
              console.log(
                '✅ [Request Interceptor] Firebase token attached to upload request',
              );
            } else if (config.url?.includes('/auth/me')) {
              if (__DEV__) {
                console.log(
                  '✅ [Request Interceptor] Firebase token attached to /auth/me request',
                );
              }
              if (__DEV__) {
                console.log(
                  `📤 [Request Interceptor] Making request to: ${config.baseURL}${config.url}`,
                );
              }
              if (__DEV__) {
                console.log(
                  `📤 [Request Interceptor] Method: ${config.method?.toUpperCase()}`,
                );
              }
              if (__DEV__) {
                console.log(
                  `📤 [Request Interceptor] Has Authorization header: ${!!config
                    .headers.Authorization}`,
                );
              }
              if (__DEV__) {
                console.log(
                  `📤 [Request Interceptor] Token length: ${token.length} characters`,
                );
              }
            }
          }
          ...
          if (__DEV__) {
            console.warn(
              `⚠️ [Request Interceptor] Failed to get Firebase token for request: ${config.url}`,
            );
        ...
        if (tokenError.code === 'auth/network-request-failed') {
          if (__DEV__) {
            console.warn(
              `⚠️ [Request Interceptor] Network error getting Firebase token for: ${config.url}`,
            );
            console.warn(
              '   Request will proceed without token (backend will handle 401)',
            );
          }
          ...
          if (__DEV__) {
            console.error(
              `❌ [Request Interceptor] Error getting Firebase token for ${config.url}:`,
              tokenError.code || tokenError.message,
            );
          }
        }
      }
      ...
      if (__DEV__) {
        console.warn(
          `⚠️ [Request Interceptor] No authenticated user found for request: ${config.url}`,
        );
        console.warn(
          '   Request will proceed without Authorization header (backend will handle 401)',
        );
      }
    }
  } catch (error: any) {
    if (__DEV__) {
      console.error(
        `❌ [Request Interceptor] Error in auth interceptor for ${config.url}:`,
        error.message || error,
      );
    }
...
api.interceptors.response.use(
  ...
  async error => {
    ...
    const isProfile404 =
      ...
      url.includes('/consultant-flow/profiles') &&
      !url.includes('/availability');
    const isUser404 = status === 404 && url.includes('/auth/users/');
    const isService404 =
      ...
      try {
        ...
        if (__DEV__) {
          console.log('🔄 Retrying request with preserved config:', {
            ...
            dataType:
              retryConfig.data?.constructor?.name || typeof retryConfig.data,
          });
        }
        ...
        if (__DEV__) {
          console.log(
            '✅ API retry succeeded, original error suppressed:',
            url,
          );
        }
      ...
      if (__DEV__) {
        ...
        if (shouldLog) {
          ...
          if (isNgrokConnError) {
            if (__DEV__) {
              console.warn(
                '⚠️ Backend unavailable (ngrok connection error):',
                url,
              );
            }
            if (__DEV__) {
              console.warn(
                '   The backend server at localhost:4000 is not running or ngrok cannot connect.',
              );
            }
            if (__DEV__) {
              console.warn(
                '   (Subsequent backend unavailable errors will be suppressed to reduce console noise)',
              );
            }
          } else {
            if (__DEV__) {
              console.warn(
                ...
                url,
                `Status: ${status || error.code || 'Connection refused'}`,
              );
            }
            if (__DEV__) {
              console.warn(
