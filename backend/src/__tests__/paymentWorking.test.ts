// Working payment tests with proper authentication mocking
...
describe('Job Posting Payment Working Tests', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('Payment Recording', () => {
    it('should record payment with correct data', async () => {
      // The payment is already being recorded by the main mock, so we just verify it works
      const response = await request(app)
        .post('/payment/job-posting/confirm')
        .send({ paymentIntentId: 'pi_test_123' })
        .expect(200);
  ...
  describe('Error Handling', () => {
    it('should handle Stripe errors gracefully', async () => {
      ...
      getStripeClient().paymentIntents.create.mockReset();
